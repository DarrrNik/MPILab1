...
int main(int argc, char* argv[])
{
    ...
    int localCols = DistributeMatrixBlocks(rows, cols, matrix, vector, commRank, commSize, &localMatrix, &localVector);

    MPI_Barrier(MPI_COMM_WORLD);
    double start = MPI_Wtime();

    MultiplyMatrixVectorBlocks(rows, localCols, localMatrix, localVector, &localResult);
