...
#mpic++ -O3 -o executables/MPILab1_task2-block mpilab.cpp block.cpp -lm
#mpic++ -O3 -o executables/MPILab1_task2-row mpilab.cpp row.cpp -lm
#mpic++ -O3 -o executables/MPILab1_task2-col mpilab.cpp col.cpp -lm

#если не компилируется из-за генераторов или компиляторов то надо дописать "cmake .. -G "твой генератор" -D CMAKE_C_COMPILER=твой/си/компилятор -D CMAKE_CXX_COMPILER=твой/спп/компилятор"
cmake .. 
cmake --build .
...
EXECUTABLES=(MPILab1_task2-block MPILab1_task2-row MPILab1_task2-col)
    ...
    for processes in "${PROCESSES_LIST[@]}"; do
        for exec in "${EXECUTABLES[@]}"; do
            ...
            if [ $processes -eq 1 ]; then
                result=$(./executables/$exec $size 2>&1 | grep "CSV:" | sed 's/CSV://')
                echo "$result" >> $RESULTS_FILE
                echo "   Result: $result"
            else
                result=$(mpirun -np $processes ./executables/$exec $size 2>&1 | grep "CSV:" | sed 's/CSV://')
                echo "$result" >> $RESULTS_FILE
                echo "   Result: $result"
            fi
